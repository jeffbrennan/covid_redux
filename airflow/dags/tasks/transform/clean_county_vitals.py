def write_vitals_to_prod():
    import sqlite3
    from datetime import datetime as dt
    import pandas as pd
    conn_prod = sqlite3.connect('db/prod.db')

    df = pd.read_parquet('data_dump/county_vitals.parquet')
    ...
    df.to_sql('county_vitals', con=conn_prod, if_exists='append', index=False)

    tbl_max_date = max(df['Date'])
    update_tbl_metadata('county_vitals', tbl_max_date)
...
def run_diagnostics():
    ...
    result_was_successful = result["success"]
    return result_was_successful


def clean_staging_vitals():
    import pandas as pd
    import sqlite3
    conn_prod = sqlite3.connect('db/prod.db')
    ...
    cleaned_vitals = clean_county_vitals(vitals_combined)
    cleaned_vitals.to_parquet('./data_dump/cleaned_vitals.parquet', index=False)